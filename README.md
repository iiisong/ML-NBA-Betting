...
Initial attempts of our model tried to predict game winners, but we had low quality data, it wasn’t scalable, and was more of a classification. It was also hard to extrapolate it to use with betting terminologies. The advantages of using score differentials was that it was scalable and translates directly to betting data in their spread. We did training and testing on a major part of the database, and then ran a gambling simulation on the rest. The projected outcome from the following betting strategy used the predicted score differential we got from our SVR model. We can simulate primitive betting by betting a constant value of 100 every time we decide to take a bet. This averaged around $20 of profit per game following this strategy in our testing.
